...
namespace OrionApiSdk.Tests
{
    ...
    public class PortfolioTests : TestBase
    {
        ...
        [TestMethod]
        public void Can_Update_Transaction()
        {
            Authenticate();
            ...
            var actual = OrionApi.Portfolio.Transaction(Id);
            Assert.IsTrue(actual != null);
            var noUnits = actual.Portfolio.NoUnits;
            actual.Portfolio.NoUnits = noUnits - 1;
            var updated = OrionApi.Portfolio.Transactions(actual);
            Assert.IsTrue(updated != null);
            Assert.IsTrue(updated.Portfolio.NoUnits.Equals(noUnits - 1));
        }
        ...
        [TestMethod]
        public void RegistrationVerbose_CanIncludeOptions()
        {
            var Id = 1;
            Authenticate();
            var rep = OrionApi.Portfolio.RegistrationVerbose(Id);
            Assert.IsNotNull(rep);
            Assert.IsNotNull(rep.portfolio);
            rep = OrionApi.Portfolio.RegistrationVerbose(Id, beneficiaries: true);
            Assert.IsNotNull(rep.beneficiaries);
            rep = OrionApi.Portfolio.RegistrationVerbose(Id, suitability:true);
            Assert.IsNotNull(rep.suitability);
            //notes seems to take too long
            //rep = OrionApi.Portfolio.RegistrationVerbose(Id, notes: true);
            //Assert.IsNotNull(rep.notes);
            rep = OrionApi.Portfolio.RegistrationVerbose(Id,  userdefinedfields: true);
            Assert.IsNotNull(rep.userDefinedFields);
            rep = OrionApi.Portfolio.RegistrationVerbose(Id, entityoptions: true);
            Assert.IsNotNull(rep.entityOptions);
        }
